...
contract PreSaleGENc is ReentrancyGuard {
    ...
    uint256 public constant MAX_PURCHASE_GENc = 5_000_000_000 * 1e18;
    uint256 public constant MAX_HOLD_PER_WALLET = 10_000_000_000 * 1e18;
    uint256 public constant MIN_GENc_BUY = 40_000 * 1e18;
    uint256 public constant TOTAL_STAGES = 5;

    uint256 public constant MIN_HOLDING_B100D = 150_000_000 * 1e18;
    uint256 public constant BASIC_PAYOUT_AMOUNT = 150_000 * 1e18;
    ...
    address payable public constant walletDevG = payable(0xF744D5516026abb2F62A2D37b8846488894B5485);
    address payable public constant walletTeamG = payable(0xBBb1d5323e89bafCF41e1ff1695ce4bE7b07eea5);
    address payable public constant walletLiquidityPool = payable(0x436945Ec6b1d8d64b6081d403B704d6850BbA71C);
    address payable public constant walletMarketing = payable(0x49a94bc3a17E857dC2f0DE1fd4Dc7cdF8115967a);
    address payable public constant walletCommunity = payable(0xBC6b28715e4c9fe375110ce1D894dFDE41F04D4C);
    address payable public constant walletProjectExpansion = payable(0x5718dAacFe5dEbd008715aFB492CbdcfC8e14b53);
    address payable public constant walletLevelUpVision = payable(0xF637f2cfb94155D0fDf38D867d3B7A09b6C1d69e);
    address payable public constant walletBurn = payable(0x544d271A6148bC360c881fB13DcDd184db73e00b);
    address payable public constant walletSecureAudit = payable(0x3B9fA0D1069CE2c77b47116EF0bFee459FeA2F28);
    address payable public constant walletDividend = payable(0xF20b08f5912F88248a6B14641240E47e6D7Ecf43);
    address payable public constant walletGENcGasReserve = payable(0xE1a86188C2FCc15B4D92526bC5bb4694De2EC9b3);
    ...
    event RemovedFromB100DWhitelist(address indexed user, uint256 remainingPayout);
    ...
    constructor() {
        owner = msg.sender;
        finalBBotAddress = 0x8E872e6DfC37BFCbe05D6A970eeb8894dF11E528;
        oracleAddress = 0x088480533422d2fDdB91a3B0B4f7b9037a6e2fdC;
        whitelistBotAddress = 0x1f3234375feF667D63574eE3279fEe67dBBf8973;
        token = IERC20(0xb0ad142296952305679f691D646B3F9e3d589349);
        
        stages[1] = Stage(104_000_000_000 * 1e18, 0.00001 * 1e18, 4, 0, 0, 0, true);     // 100B + 4%
        stages[2] = Stage(64_800_000_000 * 1e18, 0.00002 * 1e18, 8, 0, 50 days, 0, false);  // 60B + 8%
        stages[3] = Stage(65_400_000_000 * 1e18, 0.00003 * 1e18, 9, 0, 50 days, 0, false);  // 60B + 9%
        stages[4] = Stage(66_000_000_000 * 1e18, 0.00004 * 1e18, 10, 0, 50 days, 0, false); // 60B + 10%
        stages[5] = Stage(57_500_000_000 * 1e18, 0.00005 * 1e18, 15, 0, 0, 0, false);    // 50B + 15%
    }

    function updateBNBPrice(uint256 _bnbPrice) external onlyOracle {
        require(block.timestamp >= lastPriceUpdateTime + 5 minutes, "Only 1 min updates");
        bnbPrice = _bnbPrice;
    ...
    function buyTokens() external payable nonReentrant {
        ...
        if (tokensLeft < MIN_GENc_BUY) {
            _closeStage(currentStageId, true); // automatyczne zamknięcie
            revert("Stage exhausted (<50k GENc left)");
        }
    ...
    function forceCloseStageByOwner(uint256 _stageId) external onlyOwner {
        ...
        uint256 tokensLeft = stage.tokensAvailable - stage.tokensSold;
        require(tokensLeft < 50000 * 1e18, "Too many tokens left");

        emit ForceStageCloseByOwner(_stageId, tokensLeft, 50000 * 1e18, block.timestamp);
